...
public class Map {
    ...
    public void draw(GraphicsContext ctx, int characterX, int characterY) {
        int mapOffsetX = characterX - 5;
        int mapOffsetY = characterY - 5;
        ...
        for (int x = mapOffsetX; x < mapOffsetX + 11; x++) {
            for (int y = mapOffsetY; y < mapOffsetY + 11; y++) {
                ...
                ctx.drawImage(imageTiles, sheetOffsetX, sheetOffsetY, 64, 64, screenX, screenY, 64, 64);
                screenY += 64;
            }
            screenX += 64;
            screenY = 0;
